...
void Shader::setMat3(const std::string& name, glm::mat3 val) {
    glUniformMatrix3fv(glGetUniformLocation(id, name.c_str()), 1, GL_FALSE, reinterpret_cast<const GLfloat*>(glm::value_ptr(val)));
}

void Shader::setMat4(const std::string& name, glm::mat4 val) {
    glUniformMatrix4fv(glGetUniformLocation(id, name.c_str()), 1, GL_FALSE, reinterpret_cast<const GLfloat*>(glm::value_ptr(val)));
}
...
std::string Shader::defaultDirectory = "";
...
char *Shader::loadShaderSrc(bool includeDefaultHeader, const char* filePath) {
    ...
    if (includeDefaultHeader) {
        ...
        memcpy(ret, Shader::defaultHeaders.str().c_str(), cursor);
    }
    else {
